...
from settings import RIDE_SCHEMA, CONSUME_TOPIC_RIDES_CSV, TOPIC_WINDOWED_PICKUP_LOC_ID_COUNT
...
if __name__ == "__main__":    
    ...
    # df_consume_stream = read_from_kafka(consume_topic='rides_csv')
    ...
    df_trip_count_by_pickup_loc_id = op_groupby(df_rides, ['pickup_loc_id'])
    df_trip_count_by_pickup_date_pickup_loc_id = op_windowed_groupby(df_rides, window_duration="10 minutes",
                                                                 slide_duration='5 minutes')

    # write the output out to the console for debugging / testing
    sink_console(df_trip_count_by_pickup_loc_id)
    # write the output to the kafka topic
    df_trip_count_messages = prepare_df_to_kafka_sink(df=df_trip_count_by_pickup_date_pickup_loc_id,
                                                      value_columns=['count'], key_column='pickup_loc_id')
    kafka_sink_query = sink_kafka(df=df_trip_count_messages, topic=TOPIC_WINDOWED_PICKUP_LOC_ID_COUNT)
